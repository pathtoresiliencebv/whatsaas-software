...
const starterNodes: Node<WhatsAppNodeData>[] = [
  {
    ...
    data: {
      ...
      badge: 'Start Node',
      prompt: 'A new WhatsApp message enters this automation. Read the chat context and decide the next best response.',
    },
  },
  {
    id: 'qualify',
    ...
    data: {
      ...
      badge: 'Agent Node',
      prompt: 'Ask one helpful question, collect the customer need, and answer with a warm concise WhatsApp message.',
    },
  },
  {
    id: 'global',
    ...
    data: {
      ...
      title: 'Global Rules',
      badge: 'Global Node',
      prompt: 'Stay on brand, keep replies short, use available customer fields, and never invent missing order or payment details.',
    },
  },
  {
    id: 'handoff',
    ...
    data: {
      ...
      title: 'Human Handoff',
      badge: 'Action Node',
      prompt: 'If the customer asks for a human, mark the chat for follow-up and write a short handoff summary.',
    },
  },
];

const starterEdges: Edge[] = [
  { id: 'start-qualify', source: 'start', target: 'qualify', type: 'whatsappAgentEdge', data: { label: 'Reply' } },
  { id: 'start-handoff', source: 'start', target: 'handoff', type: 'whatsappAgentEdge', data: { label: 'Escalate' } },
  { id: 'qualify-handoff', source: 'qualify', target: 'handoff', type: 'whatsappAgentEdge', data: { label: 'Needs human' } },
];
      ...
      lead: [
        {
          ...
          prompt: 'Ask what the customer wants to achieve, collect name, company, and preferred next step. Keep the reply short and warm.',
        },
        {
          kind: 'condition',
          ...
          prompt: 'Check whether the customer has a clear use case, timeline, and interest in a demo. Route unclear chats back to one clarifying question.',
        },
        {
          kind: 'handoff',
          title: 'Sales Handoff',
          ...
          prompt: 'Summarize the lead, add the hot lead tag, and assign the conversation to sales with the collected details.',
        },
      ],
      support: [
        {
          ...
          label: 'Triage',
          position: startPosition,
          prompt: 'Ask for the account, issue, urgency, and one screenshot or example when useful. Keep the customer reassured.',
        },
        {
          kind: 'condition',
          ...
          label: 'Severity',
          position: { x: startPosition.x + 360, y: startPosition.y + 180 },
          prompt: 'If the issue is simple, answer with the next step. If it is billing, outage, or access related, escalate to a human teammate.',
        },
        {
          kind: 'handoff',
          ...
          label: 'Escalate',
          position: { x: startPosition.x + 720, y: startPosition.y + 320 },
          prompt: 'Create a concise support summary with customer, issue, urgency, and attempted steps, then pause automation.',
        },
      ],
      booking: [
        {
          kind: 'message',
          title: 'Collect Booking Need',
          label: 'Booking',
          position: startPosition,
          prompt: 'Ask what the customer wants to schedule, their preferred day/time, timezone, and contact details.',
        },
        {
          kind: 'condition',
          title: 'Has Booking Details?',
          label: 'Details complete',
          position: { x: startPosition.x + 360, y: startPosition.y + 180 },
          prompt: 'Check whether date, time, contact details, and purpose are clear. If not, ask one missing detail at a time.',
        },
        {
          kind: 'message',
          ...
          label: 'Confirm',
          position: { x: startPosition.x + 720, y: startPosition.y + 320 },
          prompt: 'Confirm the proposed appointment details and tell the customer a teammate will follow up if manual confirmation is needed.',
        },
      ],
    };
    ...
    <div className="flex h-screen min-h-[780px] flex-col overflow-hidden bg-[#050705] text-white">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-[#202820] bg-[#151a16] px-6">
        ...
        <div className="flex items-center gap-3">
          <ToolbarButton muted><RefreshCw className="h-4 w-4" /> v1 Draft</ToolbarButton>
          {dirty && <span className="rounded-md border border-amber-500/50 bg-amber-500/10 px-3 py-2 text-sm font-semibold text-amber-300">Unsaved changes</span>}
          {savedFlash && <span className="rounded-md border border-[#35c45f]/40 bg-[#35c45f]/10 px-3 py-2 text-sm font-semibold text-[#78df94]">Saved</span>}
          <ToolbarButton muted onClick={runTest}><Bot className="h-4 w-4" /> Test Agent</ToolbarButton>
      ...
      <div className="flex min-h-0 flex-1">
        <div ref={flowWrapperRef} onContextMenu={handleCanvasContextMenu} className="relative min-w-0 flex-1 overflow-hidden bg-[#050505]">
          ...
          <div className="pointer-events-none absolute left-1/2 top-5 z-[60] flex -translate-x-1/2 items-center gap-2 rounded-2xl border border-[#303832] bg-[#151a16]/95 p-2 text-zinc-200 shadow-2xl shadow-black/30 backdrop-blur">
            <span className="px-2 text-xs font-semibold text-zinc-400">Add to flow</span>
            <PaletteButton title="Message" onClick={() => addNode('message', getOffsetPosition(nodes, selectedNodeId, 360, 160), selectedNodeId)}>
              <MessageSquare className="h-4 w-4" />
            </PaletteButton>
            <PaletteButton title="Condition" onClick={() => addNode('condition', getOffsetPosition(nodes, selectedNodeId, 0, 260), selectedNodeId)}>
              <Zap className="h-4 w-4" />
            </PaletteButton>
            <PaletteButton title="Human handoff" onClick={() => addNode('handoff', getOffsetPosition(nodes, selectedNodeId, 360, 300), selectedNodeId)}>
              <Bot className="h-4 w-4" />
            </PaletteButton>
            <PaletteButton title="End chat" onClick={() => addNode('end', getOffsetPosition(nodes, selectedNodeId, 0, 360), selectedNodeId)}>
          ...
          <div className="absolute bottom-6 left-8 z-[60] flex gap-2 rounded-xl border border-[#303832] bg-[#151a16]/90 p-1.5 shadow-xl backdrop-blur">
            <CanvasButton title="Add step" onClick={openAddMenuNearSelected}><Plus className="h-4 w-4" /></CanvasButton>
            <CanvasButton title="Add condition" onClick={() => addNode('condition', getOffsetPosition(nodes, selectedNodeId, 0, 260), selectedNodeId)}><Zap className="h-4 w-4" /></CanvasButton>
            <CanvasButton title="Add reply" onClick={() => addNode('message', getOffsetPosition(nodes, selectedNodeId, 360, 160), selectedNodeId)}><RefreshCw className="h-4 w-4" /></CanvasButton>
            <CanvasButton title="Delete selected" onClick={deleteSelectedNode} disabled={selectedNodeId === 'start'}><Trash2 className="h-4 w-4" /></CanvasButton>
          </div>
        ...
        <aside className="relative w-[410px] shrink-0 border-l border-[#202820] bg-[#080a08] p-4">
          <div className="mb-8 grid grid-cols-2 rounded-lg bg-[#141814] p-1">
            ...
            >
              <Bot className="mr-2 inline h-4 w-4" /> Agent Preview
            </button>
          </div>
          ...
          <div className="flex h-[calc(100%-4rem)] flex-col rounded-xl border border-[#1f261f] bg-[#050705] p-5">
            {!testRunning ? (
              <div className="flex flex-1 flex-col items-center justify-center text-center">
                ...
                <h2 className="font-bold">Test this WhatsApp agent</h2>
                <p className="mt-3 text-sm leading-6 text-zinc-400">
                  Run a browser chat simulation to inspect the reply, flow path, and handoff behavior before connecting it to WhatsApp.
                </p>
                <ToolbarButton onClick={runTest} className="mt-6"><Play className="h-4 w-4" /> Run Test</ToolbarButton>
              </div>
            ) : (
              <div className="flex h-full flex-col">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="font-bold"><MessageSquare className="mr-2 inline h-4 w-4" /> Live Test Chat</h2>
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="max-w-[88%] rounded-2xl rounded-bl-md border border-[#35c45f]/30 bg-[#0f2114] px-4 py-3 text-zinc-100">
                    Thanks for reaching out. I can help with that. What would you like to automate first: support, sales, or follow-up?
                    <p className="mt-2 text-xs text-[#78df94]">WhatsApp Agent</p>
                  </div>
                </div>
                <div className="mt-auto">
                  <p className="mb-4 text-center text-sm font-semibold text-[#78df94]">Connected</p>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-black/70 px-8 py-12 backdrop-blur-sm">
      <div className="w-[min(960px,calc(100vw-64px))] rounded-xl border border-[#303830] bg-[#080a08] p-6 shadow-2xl">
        <div className="mb-8 flex items-start justify-between">
          <div>
            <h2 className="text-xl font-bold">Edit {node.data.title}</h2>
            <p className="mt-1 text-sm text-zinc-400">Configure this WhatsApp workflow node.</p>
          </div>
        ...
        <div className="space-y-5">
          ...
          <Field label="Node Type">
            ...
            >
              ...
              <option value="condition">Condition / Global</option>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <ToolbarButton muted onClick={onClose}>Cancel</ToolbarButton>
            <ToolbarButton onClick={() => onSave(draft)}>Save Node</ToolbarButton>
          </div>
        </div>
      </div>
    </div>
...
function AddNodeMenu({
  ...
  const items: Array<{ kind: WhatsAppNodeData['kind']; title: string; description: string; icon: ReactNode }> = [
    { kind: 'message', title: 'Message agent', description: 'Send or draft a WhatsApp reply', icon: <MessageSquare className="h-4 w-4" /> },
    { kind: 'condition', title: 'Condition', description: 'Route by intent, tag, or field', icon: <Zap className="h-4 w-4" /> },
    { kind: 'handoff', title: 'Human handoff', description: 'Assign to a teammate', icon: <Bot className="h-4 w-4" /> },
    { kind: 'end', title: 'End chat', description: 'Close the automation path', icon: <CheckCircle2 className="h-4 w-4" /> },
  ];
  ...
  return (
    ...
    >
      <div className="mb-1 flex items-center justify-between px-2 py-1.5">
        <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Add step</p>
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <button
            key={item.kind}
            type="button"
            aria-label={`Context menu add ${item.title}`}
      ...
      <div className="mt-2 border-t border-[#252d26] pt-2">
        <p className="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-zinc-500">Quick flows</p>
        <button type="button" onClick={() => onTemplate('lead')} className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm text-zinc-300 hover:bg-[#35c45f]/10 hover:text-white">
          Lead qualifier <span className="text-xs text-zinc-500">3 steps</span>
        </button>
        <button type="button" onClick={() => onTemplate('support')} className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm text-zinc-300 hover:bg-[#35c45f]/10 hover:text-white">
          Support triage <span className="text-xs text-zinc-500">3 steps</span>
        </button>
        <button type="button" onClick={() => onTemplate('booking')} className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm text-zinc-300 hover:bg-[#35c45f]/10 hover:text-white">
          Booking flow <span className="text-xs text-zinc-500">3 steps</span>
        </button>
      </div>
      <div className="mt-2 border-t border-[#252d26] pt-2">
        <button type="button" onClick={onDuplicate} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm text-zinc-300 hover:bg-white/5">
          <Plus className="h-4 w-4" /> Duplicate selected
        </button>
...
function normalizeEdges(value: unknown): Edge[] {
  ...
  return rawEdges.map((edge: any, index) => ({
    ...
    type: 'whatsappAgentEdge',
    data: { label: edge?.data?.label || edge?.label || 'Continue' },
  }));
}
...
function badgeForKind(kind: WhatsAppNodeData['kind']) {
  if (kind === 'start') return 'Start Node';
  if (kind === 'condition') return 'Global Node';
  if (kind === 'handoff') return 'Action Node';
  if (kind === 'end') return 'End Node';
  return 'Agent Node';
}

function titleForKind(kind: WhatsAppNodeData['kind']) {
  if (kind === 'start') return 'Incoming WhatsApp';
  if (kind === 'condition') return 'Global Rules';
  if (kind === 'handoff') return 'Human Handoff';
  if (kind === 'end') return 'Close Chat';
  return 'Qualify and Reply';
}

function promptForKind(kind: WhatsAppNodeData['kind']) {
  if (kind === 'start') return 'A new WhatsApp message enters this automation. Read the chat context and choose the next step.';
  if (kind === 'condition') return 'Apply team rules, tone, tags, and customer context before writing a reply.';
  if (kind === 'handoff') return 'Escalate to a human teammate with a concise summary when the chat needs manual attention.';
  if (kind === 'end') return 'Close the conversation politely and leave the customer with a clear next step.';
  return 'Write a helpful WhatsApp reply and ask one focused follow-up question.';
}

function edgeLabelForKind(kind: WhatsAppNodeData['kind']) {
  if (kind === 'condition') return 'Check';
  if (kind === 'handoff') return 'Escalate';
  if (kind === 'end') return 'End';
  return 'Continue';
}
