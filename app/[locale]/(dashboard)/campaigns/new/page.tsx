...
export default function NewCampaignPage() {
  ...
  const wabaInstances = (Array.isArray(instances) ? instances : []).filter((i: any) => isOfficialWhatsAppIntegration(i.integration));
  ...
  const generateOutline = () => {
    ...
    if (!name) setName(cleanPrompt.slice(0, 54));
        toast.success('Campagne-opzet gemaakt');
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Campagne maken mislukt');
    ...
    <div className="relative min-h-full overflow-hidden bg-[#f7f7f5] text-zinc-950 dark:bg-[#151719] dark:text-white">
      ...
      <header className="relative z-10 flex h-16 items-center justify-between px-4">
        <div className="flex items-center rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-[#2d332f] dark:bg-[#101311]">
          ...
          <div className="px-5 text-sm font-semibold">{name || 'Campagne zonder titel'}</div>
        </div>
        ...
        <section className="flex flex-1 items-center justify-center py-8">
          <div className="grid w-full items-start gap-5 xl:grid-cols-[300px_minmax(0,1fr)_360px]">
            <aside className="rounded-3xl border border-zinc-200 bg-white/90 p-4 shadow-sm backdrop-blur dark:border-[#2d332f] dark:bg-[#101311]/90">
              <p className="mb-4 text-xs font-bold uppercase tracking-[0.18em] text-zinc-500">Campagneflow</p>
              <StepButton step={1} active={step === 1} done={step > 1} title="Details" description="Naam, afzender, planning" onClick={() => setStep(1)} />
              <StepButton step={2} active={step === 2} done={step > 2} title="Leads" description={`${leads.length || 0} contacten geladen`} onClick={() => setStep(2)} />
              <StepButton step={3} active={step === 3} done={false} title="Template" description={selectedTemplate ? selectedTemplate.name : 'Berichtinhoud'} onClick={() => setStep(3)} />
              <div className="mt-5 rounded-2xl bg-[#ecf9ef] p-4 text-sm text-[#106b2d] dark:bg-[#35c45f]/10 dark:text-[#8de7a5]">
                ...
                Maak een concept, voeg leads toe, kies een goedgekeurde template en start alles vanuit één duidelijke flow.
              </div>
            </aside>
            ...
            <section className="rounded-[28px] border border-zinc-200 bg-white/92 p-5 shadow-xl shadow-zinc-900/5 backdrop-blur dark:border-[#2d332f] dark:bg-[#101311]/92 dark:shadow-black/30">
              <div className="mb-5 flex items-center justify-between gap-4">
                <div>
                  <h1 className="text-2xl font-bold">Nieuwe campagne</h1>
                  <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Bouw een uitgaande WhatsApp-campagne vanuit één flow.</p>
                </div>
                <StatusChip>{step === 1 ? 'Instellen' : step === 2 ? 'Doelgroep' : 'Inhoud'}</StatusChip>
              </div>

              {step === 1 && (
                <div className="space-y-5">
                  <BuilderCard icon={MessageSquare} title={t('details.title')} description="Stel de campagnebasis in voordat je leads kiest.">
                    <div className="space-y-5">
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <BuilderCard icon={LayoutTemplate} title={t('content.title')} description="Kies een goedgekeurde WhatsApp-template en controleer het eindbericht.">
                      ...
                      <div className="space-y-4">
                        ...
                        {selectedTemplate ? (
                          ...
                          <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-500 dark:border-[#2d332f] dark:bg-[#151916] dark:text-zinc-400">
                            Nog geen goedgekeurde template geselecteerd.
                          </div>
                        )}
                      </div>
            ...
            <aside className="rounded-3xl border border-zinc-200 bg-white/90 p-4 shadow-sm backdrop-blur dark:border-[#2d332f] dark:bg-[#101311]/90">
              <p className="mb-4 text-xs font-bold uppercase tracking-[0.18em] text-zinc-500">Live samenvatting</p>
              <SummaryRow icon={MessageSquare} label="Campagne" value={name || 'Zonder titel'} />
              <SummaryRow icon={CalendarClock} label="Planning" value={scheduledAt || 'Als concept versturen'} />
              <SummaryRow icon={FileSpreadsheet} label="Leads" value={`${leads.length} geladen`} />
              <SummaryRow icon={LayoutTemplate} label="Template" value={selectedTemplate?.name || 'Niet geselecteerd'} />
              <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-[#2d332f] dark:bg-[#151916]">
                <p className="text-sm font-bold">Campagne genereren</p>
                <p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-zinc-400">Beschrijf het aanbod en Kyrn zet de campagnebasis klaar.</p>
              </div>
            </aside>
          </div>
        </section>
        ...
        <div className="pointer-events-none fixed bottom-6 left-0 right-0 z-40 flex justify-center px-6 lg:left-[250px]">
          <div className="pointer-events-auto flex w-full max-w-[640px] items-center gap-2 rounded-3xl border border-zinc-200 bg-white p-2 shadow-2xl shadow-zinc-900/10 dark:border-[#2d332f] dark:bg-[#101311] dark:shadow-black/40">
            ...
            <input
              ...
              placeholder="Maak een lanceringscampagne voor warme leads..."
